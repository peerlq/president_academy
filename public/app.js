...
function inferDoorBarrier(id) {
  ...
  for (const e of neighbors) {
    ...
    if (hasRoom && hasExit) return false;
  }
  return true;
}

function doorIsBarrier(id) {
  const n = nodes[id];
  if (!n || n.type !== 'door') return false;
  if (typeof n.barrier === 'boolean') return n.barrier;
  return inferDoorBarrier(id);
}
...
function roomRequiresDoor(roomId) {
  const room = nodes[roomId];
  if (!room || room.type !== 'room') return false;
  const neighbors = adj[roomId] || [];
  let hasCorr = false;
  let hasDoorToCorr = false;
  neighbors.forEach(e => {
    const t = nodes[e.to]?.type;
    if (t === 'corridor') hasCorr = true;
    else if (t === 'door' && doorIsBarrier(e.to) && isDoorConnectedToCorridor(e.to)) hasDoorToCorr = true;
  });
  if (hasDoorToCorr) return true;
  if (hasCorr) return false;
  return false;
}

function hasDoorBetween(a, b) {
  const neighbors = adj[a] || [];
  return neighbors.some(e => {
    const id = e.to;
    return nodes[id]?.type === 'door' && doorIsBarrier(id) && hasEdge(id, b);
  });
}

function exitRequiresDoor(exitId) {
  const ex = nodes[exitId];
  if (!ex || ex.type !== 'exit') return false;
  const doors = candidateDoorsForExit(exitId).filter(doorIsBarrier);
...
function ensureNodeFromOverlay(id) {
  ...
  nodes[id] = { x: ov.x, y: ov.y, floor: '1', type: t, name: ov.name || id, barrier: ov.barrier };
  if (!adj[id]) adj[id] = [];
  return nodes[id];
}
...
function renderOverlay() {
  ...
  overlayNodes.forEach(n => {
    ...
    let nonBarrierDoor = false;
    if (t === 'door') {
      ensureNodeFromOverlay(n.id);
      nonBarrierDoor = !doorIsBarrier(n.id);
    }
    ...
    if (nonBarrierDoor) nodeClass += ' non-barrier-door-node';
    ...
    if (nonBarrierDoor) markerClass += ' non-barrier-door';
...
function mergeOverlayNodesToGraph() {
  ...
  overlayNodes.forEach(n => {
    ...
    used.add(id);
    if (!nodes[id]) nodes[id] = { x: n.x, y: n.y, floor: '1', type: n.type || 'room', name: n.name || id, barrier: n.barrier };
    if (!adj[id]) adj[id] = [];
  });
  if (changed) {
    persistWrite('navOverlayNodes', overlayNodes);
  ...
  if (btnMark) {
    btnMark.addEventListener('click', () => {
      ...
      if (markMode) {
        ...
        hintEl.textContent = 'Разметка: кликните по карте для добавления узла';
      } else {
        btnMark.classList.remove('btn-primary');
        hintEl.textContent = 'Режим разметки выключен';
      }
      renderOverlay();
    });
  }
...
function compute() {
  ...
  if (nodes[goalId]?.type === 'room') {
    const fd = finalDoorForRoom(goalId);
    if (fd) goalLoop = [fd];
  }
    ...
    if (!bestPath) {
      ...
      if (!bestPath && !(nodes[startId]?.type === 'room' && nodes[goalId]?.type === 'room')) {
        ...
        if (!bestPath && (nodes[startId]?.type === 'exit' || nodes[startId]?.type === 'stair') && nodes[goalId]?.type === 'room') {
          ...
          const exitDoorsStart = nodes[startId]?.type === 'exit' ? candidateDoorsForExit(startId) : [];
          let sDoor = null;
          let sSnap = null;
          if (exitDoorsStart.length) {
            sDoor = sortDoorsByCorridorProximity(exitDoorsStart)[0];
            sSnap = snapDoorChainToCorridor(sDoor, null, true);
          } else {
            sSnap = snapExitToCorridor(startId, null, true);
          }
          ...
          if (sSnap && gSnap && gDoor) {
            ...
            if (pSnap && pSnap.length > 1) {
              ...
              if (sDoor) candidate.push(sDoor);
              if (sSnap !== startId && sSnap !== sDoor) candidate.push(sSnap);
  ...
  if (bestPath && bestPath.length === 1) {
    const only = bestPath[0];
    if (nodes[only]?.type === 'door' && startId !== goalId) {
      const expanded = [];
      if (hasEdge(startId, only)) expanded.push(startId);
      expanded.push(only);
      if (hasEdge(only, goalId)) expanded.push(goalId);
  ...
  if (nodes[startId]?.type === 'exit' && bestStartDoor && nodes[bestStartDoor]?.type === 'door' && hasEdge(startId, bestStartDoor)) {
    if (pathFinal[0] === bestStartDoor) {
      pathFinal = [startId, ...pathFinal];
    }
  }
  ...
  const hasCorridorInFull = geomPath.some(id => nodes[id]?.type === 'corridor');
  const isRoomExitPair =
    ((nodes[startId]?.type === 'room' && nodes[goalId]?.type === 'exit') ||
     (nodes[startId]?.type === 'exit' && nodes[goalId]?.type === 'room'));
  let simpleRoomExit = false;
  if (isRoomExitPair && !hasCorridorInFull) {
    const roomId = nodes[startId]?.type === 'room' ? startId : goalId;
    const exitId = nodes[startId]?.type === 'exit' ? startId : goalId;
    if (!roomRequiresDoor(roomId) && !hasDoorBetween(roomId, exitId)) {
      simpleRoomExit = true;
    }
  }
  if (simpleRoomExit) {
    geomPath = [startId, goalId];
  }
  if (!simpleRoomExit && nodes[startId]?.type === 'room') {
  ...
  if (!simpleRoomExit && nodes[goalId]?.type === 'room') {
    ...
    if (isCorridor(idA) && isCorridor(idB)) {
      ...
      if (baseLineId) {
        ...
        while (j + 1 < path.length && isCorridor(path[j + 1])) {
          ...
          if (lineNext && lineNext !== baseLineId) {
            const sNext = String(idNext);
            if (!/^BR_/.test(sNext) || !sNext.includes(baseLineId)) break;
          }
          j++;
        }
        ...
        if (blockIds.length >= 2) {
          ...
          if (!isLocalCorrPoint(startId) && localIds.length >= 1) {
            startId = localIds[0];
          }
...
function edgeWeight(a, b) {
  ...
  if ((ta === 'corridor' && tb === 'exit') || (ta === 'exit' && tb === 'corridor')) {
    ...
    if (exitRequiresDoor(exitId)) return Infinity;
    const hasDoorNeighbor = (adj[exitId] || []).some(e => nodes[e.to]?.type === 'door' && doorIsBarrier(e.to));
    w *= hasDoorNeighbor ? 1000 : 30;
  }
...
function candidateDoorsNearNode(nodeId) {
  ...
  if (nodes[nodeId].type === 'exit') {
    const doors = candidateDoorsForExit(nodeId);
    return doors.length ? doors : [nodeId];
  }
...
function candidateDoorsForExit(exitId) {
  const res = new Set();
  const neighbors = adj[exitId] || [];
  neighbors.forEach(e => {
    const id = e.to;
    if (nodes[id]?.type !== 'door') return;
  ...
  if (res.size) return Array.from(res);
  const vb = svg.viewBox && svg.viewBox.baseVal ? svg.viewBox.baseVal : { x: 0, y: 0, width: 800, height: 500 };
  const bb = typeof viewport.getBBox === 'function' ? viewport.getBBox() : { x: 0, y: 0, width: vb.width, height: vb.height };
  const baseDim = Math.max(bb.width, bb.height);
  const threshold = Math.max(48, 0.08 * baseDim);
  Object.keys(nodes).forEach(id => {
    if (nodes[id].type !== 'door') return;
    if (!isDoorConnectedToCorridor(id)) return;
    const d = dist(exitId, id);
    if (d <= threshold) res.add(id);
  });
  return Array.from(res);
}
function finalDoorsForRoom(roomId) {
  const doors = candidateDoorsForRoom(roomId);
  if (!doors.length) return [];
  if (doors.length === 1) return doors;
  const doorSet = new Set(doors);
  const nonFinal = new Set();
  for (const start of doors) {
    if (nonFinal.has(start)) continue;
  ...
  const finals = doors.filter(id => !nonFinal.has(id));
  if (finals.length) return finals;
  return doors;
}
function finalDoorForRoom(roomId) {
  const finals = finalDoorsForRoom(roomId);
  if (!finals.length) return null;
  if (finals.length === 1) return finals[0];
  return sortDoorsByDistance(roomId, finals)[0];
}
